...
for (type in types){
  ...
  # With a buffer
  writeRaster(conn_lyrs, overwrite = TRUE, 
              file.path(result_dir, sprintf("conn_lyrs_%s.tif", type)))
  ...
  for (nm in names(conn_lyrs)){
    ...
    fname <- file.path(typology_dir, sprintf("conn_types_%s_%s.tif", nm, type))
...
classes <- data.frame(
  ID = c(0, 1, 11, 2, 22),
  class = c("Background", "F1", "F2", "I1", "I2"),
  name = c("Background", "Facilitative area", 
           "Highly facilitative area", 
           "Impeded area",
           "Highly impeded area"))
